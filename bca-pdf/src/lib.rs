...
use regex::Regex;
use std::error::Error;
use statement_core::{ SummaryTransaction, TransactionLine, TrxType, Source };
use chrono::prelude::*;
...
pub fn parse_pdf_file_to_statement(file_path:&str) -> Result<SummaryTransaction, Box<dyn Error>> {
    ...
    while i < lines.len() {
        ...
        if should_pick {
            ...
            let line = match parse_to_transaction_line_date(s, &year, &i) {
                Ok(line) => line,
                Err(_) => {
...
fn parse_to_transaction_line_date(line: &str, year: &str, index: &usize) -> Result<TransactionLine, Box<dyn Error>> {
...
mod tests {
    ...
    fn test_parse_line() {
        ...
        let result = parse_to_transaction_line_date(line, "1997", &1).unwrap();
    ...
    fn test_parse_invalid_line() {
        let line = "1231241412";
        let result = parse_to_transaction_line_date(line, "1997", &0);
